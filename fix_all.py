...
entries_1625 = [BeautifulSoup(h, 'html.parser').div for h in entries_1625]

# Extract all old entries from the soup FIRST before altering the DOM
all_old_entries = soup.find_all('div', class_='sillok-entry')
# We need to *copy* them so they don't get destroyed or lost
all_old_entries = [copy.copy(e) for e in all_old_entries]
...
for entry in all_old_entries:
    ...
    # 4. 1628
        ...
        # If it belongs to later years (1637+), we will just leave it in the DOM!
        pass

# Now we safely decompose the sections we want to replace
for period in ['1625', '1626-1628', '1627']:
    sec = soup.find('div', attrs={'data-period': period})
    if sec:
        # also remove the section-sep after it if any
        sep = sec.find_next_sibling('div', class_='section-sep')
        if sep: sep.decompose()
        sec.decompose()

# Rebuild the HTML segments
new_1625_html = f"""
<!-- 1625 -->
<div class="period-section" data-period="1625" style="--period-color:#c9a227">
    <div class="period-section-header">
        <div class="period-year-large">1625</div>
        <div class="period-event-name">冊封</div>
        <div class="period-vol-label">卷之八 · 仁祖三年</div>
        <div class="period-subtitle">The formal investiture of the Crown Prince and related diplomatic considerations</div>
    </div>
    {"".join(str(e) for e in entries_1625)}
</div>
...
# Insert these new sections BEFORE the 1637 section
sec_1637 = soup.find('div', attrs={'data-period': '1637'})
if sec_1637:
    sec_1637.insert_before(BeautifulSoup(new_1628_html, 'html.parser'))
    sec_1637.insert_before(BeautifulSoup(new_1627_html, 'html.parser'))
    sec_1637.insert_before(BeautifulSoup(new_1626_html, 'html.parser'))
    sec_1637.insert_before(BeautifulSoup(new_1625_html, 'html.parser'))

# Add Person Pills to Sidebar
person_pills_html = f"""
...
if sidebar_divider:
    # Remove existing person filters if present
    existing_persons = soup.find('div', id='person-filters')
    if existing_persons: existing_persons.decompose()
    label = soup.find(string='人物 (Persons)')
    if label: label.parent.decompose()
    sidebar_divider.insert_before(BeautifulSoup(person_pills_html, 'html.parser'))

# Update Sidebar Periods list
nav_list = soup.find('ul', class_='period-nav-list')
if nav_list:
    # Clear existing ones for 1625, 26, 27, 28
    for btn in nav_list.find_all('button'):
        p = btn.get('data-period')
        if p in ['1625', '1626-1628', '1627', '1626', '1628']:
    ...
    """
    # Insert at beginning after 'all'
    all_btn = nav_list.find('button', attrs={'data-period': 'all'})
    if all_btn:
        all_btn.parent.insert_after(BeautifulSoup(new_navs, 'html.parser'))

# Update data-persons on all remaining entries in the soup
for e in soup.find_all('div', class_='sillok-entry'):
    if not e.has_attr('data-persons'):
        t = e.get_text()
        found = [p for p in people if p in t]
